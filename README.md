...
// Read a Continuity of Care Document (CCD) instance, which is the official sample CCD instance distributed with C-CDA 2.1 specs, with a few extensions for having a more complete document
...
// By default, FHIR DSTU2 resources are generated by setting the appropriate DAF profile URLs in meta.profile attribute of resources.
// This is configurable through the statically (i.e. globally) managed Config class, and can be turned on or off.
...
// Finally, the CCD document instance is transformed to a FHIR Bundle, where the first entry is the Composition corresponding to the ClinicalDocument,
// and further entries are the ones referenced from the Composition.
